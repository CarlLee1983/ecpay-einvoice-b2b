...
use ecPay\eInvoiceB2B\Operations\InvalidConfirm;
use ecPay\eInvoiceB2B\Parameter\ConfirmAction;
use PHPUnit\Framework\TestCase;

class InvalidConfirmTest extends TestCase
{
    private InvalidConfirm $operation;

    protected function setUp(): void
    {
        $this->operation = new InvalidConfirm(
    ...
    {
        $this->assertEquals('/B2BInvoice/InvalidConfirm', $this->operation->getRequestPath());
    }
    ...
    {
        $this->operation->reject('拒絕作廢原因');
        $content = $this->getRawContent();

        $this->assertSame('2', $content['Data']['ConfirmAction']);
        $this->assertSame('拒絕作廢原因', $content['Data']['RejectReason']);
    }
