 ...
 * 查詢折讓發票範例程式碼。
 *
 * 特店(營業人)可使用此 API 查詢發票折讓資訊。
 *
 * @see https://developers.ecpay.com.tw/?p=14968
 */
...
use ecPay\eInvoiceB2B\Queries\GetAllowance;
...
// 建立查詢折讓發票請求
$query = new GetAllowance($merchantId, $hashKey, $hashIV);

// 方式一：使用折讓號碼查詢（需搭配折讓日期）
$query
    ->salesInvoice()                         // 查詢銷項發票折讓
...
try {
    ...
    if ($response->success()) {
        ...
        if (isset($data['RtnData'])) {
            ...
            echo "折讓日期: " . ($rtnData['AllowanceDate'] ?? 'N/A') . "\n";
            echo "折讓金額: " . ($rtnData['AllowanceAmount'] ?? 'N/A') . "\n";
            echo "折讓稅額: " . ($rtnData['AllowanceTax'] ?? 'N/A') . "\n";
