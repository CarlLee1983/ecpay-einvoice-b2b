 ...
 * 作廢發票範例程式碼。
 *
 * 交易雙方因發生銷貨退回或發票內容開立錯誤，由特店(營業人)傳送作廢發票參數給綠界科技加值中心，
 * 由綠界暫存相關資料。綠界會於隔日將發票作廢後上傳至財政部電子發票整合服務平台。
 *
...
// 建立作廢發票請求
$operation = new Invalid($merchantId, $hashKey, $hashIV);

// 設定作廢參數
$operation
    ->salesInvoice()                          // 銷項發票（或使用 purchaseInvoice() 作廢進項發票）
    ->setInvoiceNumber('SA37758327')          // 發票號碼
    ->setInvoiceDate('2019-08-31')            // 發票開立日期
    ->setInvalidReason(InvalidReason::INVOICE_ERROR);  // 作廢原因

// 或使用自訂作廢原因：
// $operation->setInvalidReason('金額計算錯誤');
